...
layui
  .define(['layer', 'laytpl', 'upload', 'config'], function(i) {
    let a = '3.8.0',
      e = layui.$,
      t = layui.layer,
      n = layui.laytpl,
      imConfig = layui.config,
      l = layui.device(),
      s = 'layui-show',
      o = 'layim-this',
      d = 20,
    ...
    (c.prototype.config = function(i) {
      ...
      (c.prototype.setChatStatus = function(i) {
        let a = _();
        if (a) {
          let e = a.elem.find('.layim-chat-status');
    ...
    let u = function(i) {
        let a = {friend: '该分组下暂无好友', group: '暂无群组', history: '暂无历史会话'};
        return (
          (i = i || {}),
      ...
      (layui.data.content = function(i) {
        ...
        return (i = (i || '')
          ...
          .replace(/file\([\s\S]+?\)\[[\s\S]*?\]/g, function(i) {
            let a = (i.match(/file\(([\s\S]+?)\)\[/) || [])[1],
              e = (i.match(/\)\[([\s\S]*?)\]/) || [])[1];
            return a
              ? '<a class="layui-layim-file" href="' + a + '" download target="_blank"><i class="layui-icon">&#xe61e;</i><cite>' + (e || a) + '</cite></a>'
              : i;
          })
          ...
          // 自定义消息——facetime  加入一对一聊天
          .replace(/facetime\[([^\s]+?)\]/g, function(i) {
            return (
              '<div class="layui-unselect layui-layim-facetime" layim-event="facetime" data-facetime="' +
              i.replace(/(^facetime\[)|(\]$)/g, '') +
              '"><p><i class="layui-icon">&#xe6ed;</i><span>视频通话</span></p></div>'
            );
          })
          // 自定义消息——会诊申请
          .replace(/meeting\[([^*]+?)\]/g, function(i) {
            let msg = JSON.parse(i.replace(/(^meeting\[)|(\]$)/g, '').replace(/&quot;/g, '"'));
            console.log('meeting msg', msg);
            return (
              '<div class="layui-unselect layui-layim-meeting" layim-event="facetime" data-facetime="' +
              i.replace(/(^meeting\[)|(\]$)/g, '') +
              '"><p>' +
              msg.request_doctor +
              '邀请您参加会诊，会诊对象：' +
              ...
              '</p><p>会诊时间：' +
              msg.time +
              '</p><p>请您准时参加会诊(点我进入会诊)</p></div>'
            );
          })
          .replace(/a\([\s\S]+?\)\[[\s\S]*?\]/g, function(i) {
            let a = (i.match(/a\(([\s\S]+?)\)\[/) || [])[1],
              e = (i.match(/\)\[([\s\S]*?)\]/) || [])[1];
    ...
    var x,
      ...
      H = function(i) {
        ...
        return (
          ...
          (obj = {base: i, local: local, mine: mine, history: local.history || {}}),
          (create = function(a) {
            let t = a.mine || {},
              l = layui.data('layim')[t.id] || {},
              s = {base: i, local: l, mine: t, friend: a.friend || [], group: a.group || [], history: l.history || {}};
            (j = e.extend(j, s)),
              L(n(y).render(s)),
              (l.close || i.min) && T(),
      ...
      L = function(i) {
        return t.open({
          ...
          cancel: function(i) {
            T();
            let a = layui.data('layim')[j.mine.id] || {};
      ...
      T = function(i) {
        return (
          ...
          t.open({
            ...
            resize: !1,
            content:
              '<img src="' + (j.mine.avatar || layui.cache.dir + 'css/pc/layim/skin/logo.jpg') + '"><span>' + (i || j.base.title || '我的LayIM') + '</span>',
            ...
            success: function(i, a) {
              (b = i),
                ...
                i.on('click', function() {
                  t.close(a), x.show();
                  let i = layui.data('layim')[j.mine.id] || {};
      ...
      z = function(i) {
        ...
        if (a[0]) {
          ...
          return (
            ...
            1 !== s.find('li').length || o[0] || (d || w.css('width', 800), s.css({height: w.height()}).show(), c.css('margin-left', '200px')),
        ...
        w && !i && w.hide(),
          ...
          (N.index = t.open({
            ...
            success: function(a, l) {
              i || (k = a),
                n.minRight && t.style(l, {left: e(window).width() - a.outerWidth() - parseFloat(n.minRight)}),
      ...
      M = function(i, a) {
        return (
          ...
          (M.index = t.open({
            ...
            btn: a ? ['确认', '取消'] : ['发送申请', '关闭'],
            content: n(p).render({
              data: {name: i.username || i.groupname, avatar: i.avatar, group: i.group || parent.layui.layim.cache().friend || [], type: i.type},
              type: a,
            }),
            yes: function(e, t) {
              let n = t.find('#LAY_layimGroup'),
      ...
      O = function(i, a) {
        ...
        let n = i.index() === -1 ? 0 : i.index(),
          l = '.layim-chat',
          d = w.find(l).eq(n),
          c = w.find('.layui-layer-max').hasClass('layui-layer-maxmin');
        if (a) {
          i.hasClass(o) && O(0 === n ? i.next() : i.prev());
          let u = w.find(l).length;
          return 1 === u
            ? t.close(C)
            : (i.remove(),
              d.remove(),
              2 === u && (w.find('.layim-chat-list').hide(), c || w.css('width', '600px'), w.find('.layim-chat-box').css('margin-left', 0)),
              !1);
        }
        i
          .addClass(o)
      ...
      _ = (c.prototype.thisChat = function() {
        if (w) {
          let i = e('.layim-chat-list .' + o).index(),
      ...
      $ = function(i) {
        let a = layui.data('layim')[j.mine.id] || {},
          ...
          l = t[i.type + i.id];
        if (x) {
          let s = x.find('.layim-list-history');
          if (((i.historyTime = new Date().getTime()), (t[i.type + i.id] = i), (a.history = t), layui.data('layim', {key: j.mine.id, value: a}), !l)) {
            e[i.type + i.id] = i;
            let o = n(u({type: 'history', item: 'd.data'})).render({data: e});
      ...
      J = function(i) {
        if (((i = i || {}), window.Notification)) {
          if ('granted' === Notification.permission) {
            new Notification(i.title || '', {body: i.content || '', icon: i.avatar || 'http://tp2.sinaimg.cn/5488749285/50/5719808192/1'});
      ...
      K = function(i) {
        i = i || {};
        let a = e('.layim-chatlist-' + i.type + i.id),
          t = {},
          l = a.index();
        if (
          ...
          if (
            ...
            return (
              j.base.notice && J({title: '来自 ' + i.username + ' 的消息', content: i.content, avatar: t.avatar || i.avatar}),
        ...
        if (w) {
          ...
          let r = w.find('.layim-chat').eq(l),
            c = r.find('.layim-chat-main ul');
          i.system
            ? l !== -1 && c.append('<li class="layim-chat-system"><span>' + i.content + '</span></li>')
      ...
      V = function(i) {
        let a = x.find('.layim-tool-msgbox');
        a.find('span')
          .addClass(Y)
          .html(i);
      },
      ...
      P = function(i) {
        ...
        if (!a) {
          let s = n(u({type: i.type, item: 'd.data', index: 'friend' === i.type ? 'data.groupIndex' : null})).render({data: e});
          if ('friend' === i.type) {
            let o = l.find('>li').eq(i.groupIndex);
      ...
      Q = function() {
        ...
        a.focus(),
          a.off('keydown').on('keydown', function(i) {
            let e = layui.data('layim')[j.mine.id] || {},
              t = i.keyCode;
      ...
      ti = {
        ...
        sign: function() {
          let i = x.find('.layui-layim-remark');
        ...
        msgbox: function() {
          let i = x.find('.layim-tool-msgbox');
          return (
        ...
        about: function() {
          t.alert('版本： ' + a + '<br>版权所有：<a href="http://layim.layui.com" target="_blank">layim.layui.com</a>', {title: '关于 LayIM', shade: !1});
        },
        setSkin: function(i) {
          let a = i.attr('src'),
          ...
          layui.each(r.setSkin, function(i, e) {
            let t = (a || '').replace(layui.cache.dir + 'css/modules/layim/skin/', '');
            e && e(t, a);
          });
        },
        chat: function(i) {
        ...
        setSend: function(i, a) {
          let t = (ti.setSend.box = i.siblings('.layim-menu-box')),
            n = i.attr('lay-type');
          if ('show' === n) {
            ii(a),
              t.show().addClass(ei),
            ...
            i.addClass(o)
              .siblings()
              .removeClass(o);
            let l = layui.data('layim')[j.mine.id] || {};
        ...
        face: function(i, a) {
          let n = '',
            l = _();
          for (let s in X) n += '<li title="' + s + '"><img src="' + X[s] + '"></li>';
              ...
              0 == i.code
                ? ((i.data = i.data || {}),
                  'images' === a
                    ...
                    : 'file' === a && ai(n.textarea[0], 'file(' + (i.data.src || '') + ')[' + (i.data.name || '下载文件') + ']'),
                  D())
                : t.msg(i.msg || '上传失败');
            },
          });
        ...
        media: function(i) {
          let a = i.data('type'),
            ...
            l = _();
          t.prompt(
            {title: '请输入网络' + n[a] + '地址', shade: !1, offset: [i.offset().top - e(window).scrollTop() - 158 + 'px', i.offset().left + 'px']},
            function(i, e) {
        ...
        facetime: function(i) {
          const facetimeInfo = i.data('facetime');
          console.log('facetime i', facetimeInfo);
          // chatLog下可以点击了
          if (parent.layui) {
            const chatlogDom = parent.document.querySelector('.layui-layer.layui-layer-iframe.layui-layer-border.layui-box');
            const chatDom = parent.document.querySelector('.layui-layer.layui-layer-page.layui-box.layui-layim-chat');
            if (chatlogDom) {
              parent.layui.layer.close(parseInt(chatlogDom.id.substr(11)));
            }
            if (chatDom) {
              parent.layui.layer.close(parseInt(chatDom.id.substr(11)));
            }
            parent.window.joinFaceTime(facetimeInfo);
            parent.closeLayIM();
          } else {
            const chatlogDom = document.querySelector('.layui-layer.layui-layer-iframe.layui-layer-border.layui-box');
            const chatDom = document.querySelector('.layui-layer.layui-layer-page.layui-box.layui-layim-chat');
            if (chatlogDom) {
              t.close(parseInt(chatlogDom.id.substr(11)));
            }
            if (chatDom) {
              t.close(parseInt(chatDom.id.substr(11)));
            }
        ...
        menuHistory: function(i, a) {
          let n = layui.data('layim')[j.mine.id] || {},
            l = i.parent(),
            s = i.data('type'),
