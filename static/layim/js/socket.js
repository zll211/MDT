  ...
  var im = {
    ...
    contextMenu: function() {
      ...
      my_spread.mousedown(function(e) {
        var data = {
          ...
          target: function(ele) {
            // 当前元素
            $('.context-friend')
              .attr('data-id', ele[0].id.replace(/[^0-9]/gi, ''))
        ...
        var currentGroupidx = $(this)
          .find('h5')
          .data('groupidx'); // 当前分组id
        if (my_spread.length >= 2) {
          // 当至少有两个分组时
          var html = '<ul>';
          for (var i = 0; i < my_spread.length; i++) {
            var groupidx = my_spread
              .eq(i)
              .find('h5')
              .data('groupidx');
            if (currentGroupidx != groupidx) {
              var groupName = my_spread
                .eq(i)
                .find('h5 span')
                .html();
              html +=
                '<li class="ui-move-menu-item" data-groupidx="' +
                groupidx +
                '" data-groupName="' +
                groupName +
      ...
      $('.groupMembers > li').mousedown(function(e) {
        ...
        var data = {
          ...
          target: function(ele) {
            // 当前元素
            $('.context-group-member').attr('data-id', ele[0].id.replace(/[^0-9]/gi, ''));
        ...
        if (cachedata.mine.id !== _this.attr('id')) {
          data.menu.push(im.menuChat());
          data.menu.push(im.menuInfo());
          if (3 == e.which && $(this).attr('isfriend') == 0) {
            // 点击右键并且不是好友
        ...
        if (groupInfo.manager == 1 && cachedata.mine.id !== _this.attr('id')) {
          // 是群主且操作的对象不是自己
    ...
    initListener: function(user, pwd) {
      ...
      conn.listen({
        ...
        onPresence: function(message) {
          ...
          if (message.type === 'unsubscribe') {
          ...
          } else if (message.type === 'joinGroupNotifications') {
            // 群管理收到加群申请 将该管理员加入消息组
            console.log(message);
            $.get(
              'class/doAction.php?action=add_admin_msg',
              {
                from: message.from,
            ...
            if ($('ul[data-groupidx=' + message.from + '] #' + message.to).html()) {
              ...
              var html =
                '<li id="' +
                message.to +
                '" isfriend="0" manager="2"><img src="' +
                cachedata.mine.avatar +
                '"><span style="color:#de6039">' +
                cachedata.mine.username +
                '(' +
                cachedata.mine.id +
                ' )<i class="layui-icon" style="color:#eaa48e"></i></span></li>';
              $('ul[data-groupidx=' + message.from + ']')
                .find('li')
                .eq(0)
                .after(html);
              layui.each(cachedata.group, function(index, item) {
            ...
            $.get('class/doAction.php?action=get_one_group_data', {groupIdx: message.from}, function(res) {
              ...
              layer.open({
                ...
                title: false, // 不显示标题
                content:
                  '<div style="padding: 20px;font-size: 15px;background: #ddd;">你已成为群 <b>' +
                  data.data.groupName +
                  '(' +
                  message.from +
                  ')</b> 的管理员，快去看看吧！</div>',
            ...
            $.get('class/doAction.php?action=get_one_group_data', {groupIdx: message.from}, function(res) {
              ...
              layer.open({
                ...
                title: false, // 不显示标题
                content:
                  '<div style="padding: 20px;font-size: 15px;background: #ddd;">你已被群 <b>' +
                  data.data.groupName +
                  '(' +
                  message.from +
                  ')</b> 撤消管理员。</div>',
    ...
    addFriendGroup: function(othis, type) {
      var li = othis.parents('li') || othis.parent(),
        ...
        name = li.data('name');
      if (uid === 'undifine' || !uid) {
        var uid = othis.parent().data('id'),
          name = othis.parent().data('name');
      }
      var avatar = './uploads/person/' + uid + '.jpg';
      var isAdd = false;
      if (type === 'friend') {
        var default_avatar = './uploads/person/empty2.jpg';
        if (cachedata.mine.id == uid) {
          // 添加的是自己
          layer.msg('不能添加自己');
          return false;
        }
      ...
      parent.layui.layim.add({
        ...
        submit: function(group, remark, index) {
          ...
          if (type === 'friend') {
            $.get(
              'class/doAction.php?action=add_msg',
              {
                to: uid,
                msgType: 1,
                remark: remark,
          ...
          var options = {
            ...
            success: function(respData) {
              if (respData.data.groupid) {
                $.get(
                  'class/doAction.php?action=commitGroupInfo',
                  {
    ...
    groupMembers: function(othis, e) {
      ...
      if (othis.data('show')) {
        ...
        var members = cachedata.base.members || {},
          ul = $('#layui-layim-chat'),
          li = '',
          membersCache = {};
              ...
              if (
                ...
                for (var i = 0; i < number; i++) {
                  var friend_id = friend
                    .eq(i)
                    .attr('id')
                    .replace(/^layim-friend/g, ''); // 好友id
                  var friend_name = friend
                    .eq(i)
                    .find('span')
                    .html(); // 好友id
                  var signature = friend
                    .eq(i)
                    .find('p')
                    .html(); // 好友id
    ...
    setAdmin: function(othis) {
      var username = othis.data('id'),
        friend_avatar = othis.data('img'),
        isfriend = othis.data('isfriend'),
        name = othis.data('name'),
        gagTime = othis.data('gagtime'),
        groupidx = othis.data('groupidx');
      var options = {
        groupId: groupidx,
        username: username,
        success: function(resp) {
          $.get(
            'class/doAction.php?action=setAdmin',
            {
              groupidx: groupidx,
              memberIdx: username,
              type: 2,
            ...
            function(res) {
              ...
              if (admin.code == 0) {
                ...
                var html =
                  ...
                  '"><span style="color:#de6039">' +
                  name +
                  '</span><i class="layui-icon" style="color:#eaa48e"></i></li>';
                $('ul[data-groupidx=' + groupidx + ']')
                  .find('li')
                  .eq(0)
                  .after(html);
                im.contextMenu();
    ...
    editGroupNickName: function(othis) {
      ...
      layer.prompt({title: '请输入群名片，并确认', formType: 0, value: name[0]}, function(nickName, index) {
        $.get(
          'class/doAction.php?action=editGroupNickName',
          {
            nickName: nickName,
            memberIdx: memberIdx,
            groupIdx: groupIdx,
          ...
          function(res) {
            var data = eval('(' + res + ')');
            if (data.code == 0) {
              $('ul[data-groupidx=' + groupIdx + '] #' + memberIdx)
                .find('span')
                .html(nickName + '(' + memberIdx + ')');
              layer.close(index);
            }
            layer.msg(data.msg);
          }
    ...
    leaveGroup: function(groupIdx, list, username) {
      // list为数组
    ...
    setGag: function(options) {
      ...
      if (_this_group.find('span').html()) {
        ...
        var cont = _this_group
          .parent()
          .parent()
          .find('.layim-chat-box div')
          .eq(index);
        ...
        if (options.type === 'set' && (options.user == cachedata.mine.id || options.user === 'ALL')) {
          // 设置禁言单人或全体
          if (options.gagTime) {
        ...
        } else if (options.type === 'lift' && (options.user == cachedata.mine.id || options.user === 'ALL')) {
          // 取消禁言单人或全体
          cont.find('.layim-chat-tool').data('json', encodeURIComponent(JSON.stringify(info)));
          layui.each(cachedata.group, function(index, item) {
            if (item.id === options.groupidx) {
    ...
    menuMove: function(html) {
      return (data = {
        ...
        callback: function(ele) {
          ...
          var avatar = '../uploads/person/' + friend_id + '.jpg';
          var default_avatar = './uploads/person/empty2.jpg';
          var signature = $('.layim-list-friend')
            .find('#layim-friend' + friend_id)
            .find('p')
            .html(); // 获取签名
    ...
    menuRemove: function() {
      return (data = {
        ...
        callback: function(ele) {
          ...
          layer.confirm(
            '删除后对方将从你的好友列表消失，且以后不会再接收此人的会话消息。<div class="layui-layim-list"><li layim-event="chat" data-type="friend" data-index="0"><img src="./uploads/person/' +
              friend_id +
              '.jpg"><span>' +
              username +
              '</span><p>' +
      ...
      return (data = {
        ...
        callback: function(ele) {
          ...
          layer.prompt({title: '请输入分组名，并确认', formType: 0, value: groupName}, function(mygroupName, index) {
            if (mygroupName) {
              $.get(
                'class/doAction.php?action=editGroupName',
                {
                ...
                function(res) {
                  ...
                  if (data.code == 0) {
                    ...
                    for (var j = 0; j < friend_group.length; j++) {
                      var groupidx = friend_group
                        .eq(j)
                        .find('h5')
                        .data('groupidx');
                      if (groupidx == mygroupIdx) {
                        // 当前选择的分组
                        friend_group
                          .eq(j)
                          .find('h5')
    ...
    menuLeaveGroupBySelf: function() {
      return (data = {
        ...
        callback: function(ele) {
          ...
          avatar = othis.data('img');
          layer.confirm(
            '您真的要退出该群吗？退出后你将不会再接收此群的会话消息。<div class="layui-layim-list"><li layim-event="chat" data-type="friend" data-index="0"><img src="' +
              avatar +
              '"><span>' +
              groupname +
              '</span></li></div>',
        ...
        callback: function(ele) {
          ...
          item.hover(function() {
            var _index = item.index(this),
              gagTime = item.eq(_index).data('gag'); // 禁言时间
            $.get(
              'class/doAction.php?action=groupMemberGag',
              {
                gagTime: gagTime,
                groupidx: groupidx,
                friend_id: friend_id,
    ...
    menuLiftGroupMemberGag: function() {
      return (data = {
        ...
        callback: function(ele) {
          ...
          groupidx = ele.parent().data('groupidx');
          $.get(
            'class/doAction.php?action=liftGroupMemberGag',
            {
              groupidx: groupidx,
              friend_id: friend_id,
            },
